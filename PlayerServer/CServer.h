...
public:
    ...
    //带网络运行模式配置的初始化接口（支持 DPDK 学习链路）
    int Init(CBusiness* business, const NetRuntimeConfig& cfg, const Buffer& ip, short port);
    //阻塞主线程，维持服务运行
    int Run();
...
private:
    ...
    NetRuntimeConfig m_runtimeConfig; // 当前生效的网络运行配置
    DpdkRuntime m_dpdkRuntime; // 可选 DPDK 学习链路
};
